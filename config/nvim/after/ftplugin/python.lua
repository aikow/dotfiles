...
vim.keymap.set(
  ...
  "<localleader>r",
  [[:!python3 %<CR>]],
  { desc = "Run the file or region through the interpreter" }
)
vim.keymap.set(
  "n",
  "<localleader>l",
...
vim.keymap.set("n", "<localleader>fb", function()
  vim
    ...
    :wait()
  vim.cmd.edit()
end, { desc = "format the entire buffer using black with string processing enabled" })
...
vim.keymap.set(
  ...
  { desc = "Format the current buffer by fixing all lints using ruff" }
)
...
vim.keymap.set(
  ...
  "<localleader>fs",
  "<cmd>StringWrap<cr>",
  { desc = "format the buffer by wrapping all multi-line strings" }
)
