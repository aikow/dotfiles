...
function desktop.add_single_icon(args, label, icon, onclick)
  ...
  if not desktop_current_pos[s] then
    desktop_current_pos[s] = {
      x = (capi.screen[s].geometry.x + args.iconsize.width + args.margin.x),
      y = 40,
    }
  end
  ...
  if desktop_current_pos[s].y + totheight > capi.screen[s].geometry.height - 40 then
    desktop_current_pos[s].x = desktop_current_pos[s].x
      + args.labelsize.width
      + args.iconsize.width
      + args.margin.x
...
function desktop.add_base_icons(args)
  for _, base in ipairs(args.baseicons) do
    desktop.add_single_icon(
      ...
      function() awful.spawn(string.format("%s '%s'", args.open_with, base.onclick)) end
    )
  end
end
...
function desktop.add_dirs_and_files_icons(args)
  for _, file in ipairs(desktop.parse_dirs_and_files(args.dir)) do
    if file.show then
      ...
      local onclick =
        function() awful.spawn(string.format("%s '%s'", args.open_with, file.path)) end
