function fzf_files --description "Search the current directory. Replace the current token with the selected file paths."
  ...
  if string match --quiet -- "*/" $unescaped_exp_token && test -d "$unescaped_exp_token"
    ...
    set --prepend fzf_arguments --prompt="$unescaped_exp_token" --preview="fzf_preview_file $expanded_token{}"
    set file_paths_selected $unescaped_exp_token(fd $fd_opts 2>/dev/null | fzf_wrapper $fzf_arguments)
  else
    set --prepend fzf_arguments --query="$unescaped_exp_token" --preview='fzf_preview_file {}'
    set file_paths_selected (fd $fd_opts 2>/dev/null | fzf_wrapper $fzf_arguments)
  end
